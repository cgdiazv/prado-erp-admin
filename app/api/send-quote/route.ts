...
<body style="margin: 0; padding: 0; background-color: #f8fafc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #334155;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #f8fafc; padding: 30px 10px;">
          ...
          <tr>
            <td style="padding: 32px;">
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #f8fafc; border-radius: 12px; border: 1px solid #e2e8f0; padding: 16px; margin-bottom: 24px;">
                <tr>
                  <td width="50%" style="font-size: 12px; color: #64748b; padding-bottom: 8px;">
                    <strong>Fecha de emisión:</strong> <span style="color: #0f172a;">${quoteDate}</span>
                  </td>
                  <td width="50%" style="font-size: 12px; color: #64748b; padding-bottom: 8px;" align="right">
                    <strong>Válida hasta:</strong> <span style="color: #0f172a;">${validUntil || "30 días calendario"}</span>
                  </td>
                </tr>
                <tr>
                ...
                ${salesRepName ? `
                <tr>
                  <td colspan="2" style="font-size: 12px; color: #64748b; padding-top: 8px;">
                    <strong>Asesor Comercial:</strong> <span style="color: #0f172a;">${salesRepName}</span>
                  </td>
                </tr>
                ` : ""}
              </table>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="border-collapse: collapse; margin-bottom: 24px;">
                <thead>
                  <tr style="background-color: #f1f5f9; border-bottom: 2px solid #cbd5e1;">
                    ...
                    <th style="padding: 10px; font-size: 11px; font-weight: 700; color: #475569; text-align: right;">Precio</th>
                    <th style="padding: 10px; font-size: 11px; font-weight: 700; color: #475569; text-align: right;">Importe</th>
                  </tr>
                </thead>
                <tbody>
                  ${itemsTableRowsHtml || `<tr><td colSpan="7" style="padding: 16px; text-align: center; font-size: 12px; color: #94a3b8;">Sin ítems detallados</td></tr>`}
                </tbody>
              </table>
              ...
              ${
                notes
                  ? `
              <div style="background-color: #f8fafc; border-left: 4px solid #1b426e; padding: 12px 16px; border-radius: 4px; margin-bottom: 16px;">
                <p style="margin: 0 0 4px 0; font-size: 11px; font-weight: 700; color: #475569; text-transform: uppercase;">Notas:</p>
                <p style="margin: 0; font-size: 12px; color: #1e293b;">${notes}</p>
              </div>
              `
                  : ""
              }
              ...
              ${
                termsConditions
                  ? `
              <div style="background-color: #f8fafc; border-left: 4px solid #64748b; padding: 12px 16px; border-radius: 4px; margin-bottom: 24px;">
                <p style="margin: 0 0 4px 0; font-size: 11px; font-weight: 700; color: #475569; text-transform: uppercase;">Términos y Condiciones:</p>
                <p style="margin: 0; font-size: 12px; color: #475569;">${termsConditions}</p>
              </div>
              `
                  : ""
              }
    ...
    if (error) {
      console.error("Error al enviar cotización con Resend:", error);
      return NextResponse.json(
    ...
    return NextResponse.json({
      success: true,
      data,
      message: `Cotización ${quoteNumber} enviada correctamente a ${to}`,
    });
  } catch (err: any) {
    console.error("Error en POST /api/send-quote:", err);
    return NextResponse.json(
