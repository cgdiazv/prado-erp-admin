...
export default function VendorsModule({
  ...
  const [newVendorForm, setNewVendorForm] = useState(defaultNewVendorForm);
  ...
  // Handle autoOpenCreate from external trigger
  React.useEffect(() => {
    if (autoOpenCreate) {
  ...
  const filteredVendors = useMemo(() => {
    return vendors.filter((v) => {
      ...
      if (filterType === "macola") {
        return Boolean(v.macolaCode && v.macolaCode.trim() !== "");
      }
      return true;
    });
  ...
  // Create handler
  const handleCreateVendor = async (e?: React.FormEvent, createAnother = false) => {
    if (e) e.preventDefault();
    if (!newVendorForm.name.trim()) {
      alert("Por favor ingresa el nombre de la empresa o proveedor.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/vendors", {
        ...
        body: JSON.stringify({
          ...
          macolaCode: newVendorForm.macolaCode || `VEN-${Math.floor(100 + Math.random() * 900)}`,
          email: newVendorForm.email,
          phone: newVendorForm.phone,
          address: newVendorForm.address,
          currency: newVendorForm.currency,
        }),
      });
      ...
      if (json.success && json.data) {
        ...
        setDrawerSuccess("¡Proveedor registrado correctamente!");
        setTimeout(() => setDrawerSuccess(""), 4000);

        if (createAnother) {
          setNewVendorForm(defaultNewVendorForm);
        } else {
          setShowNewDrawer(false);
          setNewVendorForm(defaultNewVendorForm);
        }
      } else {
        alert(json.error || "Error al registrar el proveedor");
      }
    } catch (err: any) {
  ...
  const handleUpdateVendor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEditError(err.message || "Error al actualizar proveedor");
    } finally {
  ...
  const handleDeleteVendor = async () => {
    ...
    try {
      ...
      setEditError(err.message || "Error al eliminar proveedor");
    } finally {
      setEditLoading(false);
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-3.5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 self-end md:self-auto text-slate-400">
          <button
            type="button"
            onClick={() => alert("Exportando catálogo de proveedores...")}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xs">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          <h2 className="font-semibold text-sm text-slate-900">
            Directorio de Proveedores ({filteredVendors.length})
          </h2>
        </div>
      ...
      {/* ================= SIDEBAR / DRAWER: CREAR NUEVO PROVEEDOR ================= */}
      {showNewDrawer && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <aside className="relative z-50 w-full max-w-xl bg-slate-50 border-l border-slate-200 shadow-2xl flex flex-col h-full animate-in slide-in-from-right duration-200">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-2xs">
              <div>
                <h2 className="text-base font-bold text-slate-800">Añadir un nuevo proveedor</h2>
                <p className="text-xs text-slate-500">Registra un proveedor de materias primas o insumos</p>
              </div>
              <button
                type="button"
                onClick={() => setShowNewDrawer(false)}
            ...
            <form onSubmit={(e) => handleCreateVendor(e, false)} className="flex-1 overflow-y-auto p-6 space-y-4 text-xs">
              {/* SECTION 1: INFORMACIÓN BÁSICA DEL PROVEEDOR */}
              <div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-xs space-y-4">
                ...
                >
                  <span>Información básica del proveedor</span>
                ...
                {drawerSectionsOpen.basic && (
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          value={newVendorForm.rtn}
                          onChange={(e) => setNewVendorForm({ ...newVendorForm, rtn: e.target.value })}
                          className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:border-[#1b426e] text-slate-900"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          value={newVendorForm.macolaCode}
                          onChange={(e) => setNewVendorForm({ ...newVendorForm, macolaCode: e.target.value })}
                    ...
                    <div>
                      ...
                      <select
                        value={newVendorForm.currency}
                        onChange={(e) => setNewVendorForm({ ...newVendorForm, currency: e.target.value })}
                        className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:border-[#1b426e] text-slate-900"
                      >
                ...
                {drawerSectionsOpen.commercial && (
                  <div className="space-y-4 pt-2">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-700 mb-1">Línea de Crédito Otorgada ({newVendorForm.currency})</label>
                        <input
                          type="number"
                          placeholder="0.00"
                          value={newVendorForm.creditLimit || ""}
                          onChange={(e) => setNewVendorForm({ ...newVendorForm, creditLimit: parseFloat(e.target.value) || 0 })}
                          className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:border-[#1b426e] text-slate-900 font-mono"
                        />
                      </div>
                    </div>
            ...
            <div className="border-t border-slate-200 px-6 py-3.5 bg-slate-50/50 flex items-center justify-end gap-2.5">
              ...
              <div className="relative inline-flex rounded-lg shadow-sm">
                <button
                  ...
                  onClick={(e) => handleCreateVendor(e, false)}
